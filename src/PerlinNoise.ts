...
function fade(t: number): number {
    ...
    let lower: number = n >> 12;
    let upper: number = n & 0xfff;
...
export function noise2d( x, y) {
    const temp = ptable(x >> 16 & 0xff /* Unit square X */);

    const a = ptable((temp >> 8  ) + (y >> 16 & 0xff /* Unit square Y */));
    const b = ptable((temp & 0xff) + (y >> 16 & 0xff                    ));
    ...
    const c = lerp(u, grad2(a >> 8  , x, y        ), grad2(b >> 8  , x-0x10000, y        ));
    const d = lerp(u, grad2(a & 0xff, x, y-0x10000), grad2(b & 0xff, x-0x10000, y-0x10000));
